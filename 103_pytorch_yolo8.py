    ...
    if masks is not None:
        for i, mask in enumerate(masks):
            ...
            if class_id == 0:
                ...
                # マスクを膨張（膨張の度合いを大きくする）
                kernel = np.ones((Int_mask_number, Int_mask_number), np.uint8)
                binary_mask = cv2.dilate(binary_mask, kernel, iterations=2)
                
                # マスクを平滑化（平滑化の度合いを大きくする）
                binary_mask = cv2.GaussianBlur(binary_mask, (Int_mask_number, Int_mask_number), 0)
                
                # マスクを使用して黒塗り（閾値を下げる）
                image[binary_mask > Int_mask_threshold] = 0
...
# 結果の保存
output_path = r"E:\Programing\20250405_object_recognition\Picuture\output_pytorch_yolo8_4.jpg"
